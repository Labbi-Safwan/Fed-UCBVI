...
class FiniteStateFiniteActionMDP(object):
    def __init__(self, H=3, S=50, A=50, epsilon_p = 0., common=None, common_reward=None):
        super().__init__()
        self.H = H
        self.S = S
        self.A = A
        ...
        if self.common is not None:
            self.local_P = RandomSimplexVector(d = self.S, size=[self.H, self.S, self.A])
            #self.P = RandomSimplexVector(d = self.S, size=[self.H, self.S, self.A]) # transition probability shape [H S A S']
            self.P = (1 - self.epsilon_p)*self.common + self.epsilon_p*self.local_P
        else: self.P = RandomSimplexVector(d = self.S, size=[self.H, self.S, self.A])
        
        if self.common_reward is not None: self.R = self.common_reward
        else: self.R = np.random.uniform(0., 1., size=[self.H, self.S, self.A]) # reward between [0, 1],  shape [H, S, A]
